...
export function useSudokuGame(opts: UseSudokuGameOptions = {}) {
  ...
  const loadGame = useCallback(() => {
    try {
      ...
      setBoard(applyBoardErrors(p.board));
  ...
  const useHint = useCallback(async () => {
    ...
    try {
      ...
      if (!error && data && (data.value === null || data.value === undefined) && hintLevel <= 2) {
        if (hintLevel === 1) {
          ...
          setHintCoach({
            ...
            explanation: data.explanation ?? "Seguí analizando la celda.",
          });
        }
        logHintUsage({ level: hintLevel, technique: data.technique ?? null });
        setHintsUsed((h) => h + 1);
      ...
      const updated = applyBoardErrors(next);
      setBoard(updated);
      setHintsUsed((h) => h + 1);
      setHintChain(null);
      setHistory((hist) => [
  ...
  const placeNumber = useCallback(
    (n: number) => {
      ...
      if (isNotesMode) {
        ...
        setBoard(applyBoardErrors(next));
        setHistory((h) => [
          ...h,
          {
            row,
