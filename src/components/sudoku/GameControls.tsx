import { Eraser, Lightbulb, Pencil, Undo2, Wand2 } from "lucide-react";
...
interface GameControlsProps {
  ...
  disabled?: boolean;
  /** Auto-notas (Bloque G) — desbloqueado por nivel o sin auth. */
  onAutoNotes?: () => void;
  showAutoNotes?: boolean;
}
  ...
  return (
    <div className="flex w-full max-w-[520px] flex-col items-center gap-2">
      <p className="text-center text-[11px] text-muted-foreground">
        Pistas en 3 niveles: zona → técnica → respuesta (cada uno consume 1 uso)
      </p>
      <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4">
        ...
        {showAutoNotes && onAutoNotes ? (
          <button
            type="button"
            className={cn(btn)}
            disabled={disabled}
            onClick={onAutoNotes}
            aria-label="Auto-notas: rellenar candidatos"
            title="Auto-notas"
          >
